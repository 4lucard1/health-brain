  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '20px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '4px' }}>Supplements</h2>
        ...
        <motion.button
          ...
          style={{ backgroundColor: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '14px', padding: '8px 16px', color: '#22C55E', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}
        >+ Add</motion.button>
      </div>
      ...
      <AnimatePresence>
        {addingSupp && (
          ...
          >
            <div style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[{ key: 'name', placeholder: 'Name (e.g. Vitamin D3)' }, { key: 'dose', placeholder: 'Dose (e.g. 5000 IU)' }, { key: 'timing', placeholder: 'When (e.g. Morning with food)' }].map(({ key, placeholder }) => (
                <input key={key} placeholder={placeholder} value={newSupp[key as keyof typeof newSupp]}
                  ...
                  style={{ width: '100%', backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '11px 16px', color: '#F5F5F5', fontSize: '14px', outline: 'none' }} />
              ))}
              <motion.button whileTap={{ scale: 0.98 }} onClick={addSupplement}
                style={{ padding: '13px', background: 'linear-gradient(135deg, #22C55E, #16A34A)', border: 'none', borderRadius: '14px', color: '#fff', fontSize: '14px', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 20px rgba(34,197,94,0.3)' }}>
                Save Supplement
              </motion.button>
            </div>
          </motion.div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {supplements.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9CA3AF' }}>
            ...
            <p style={{ fontSize: '15px', fontWeight: 500, color: '#F5F5F5', marginBottom: '6px' }}>No supplements yet</p>
            ...
            return (
              <motion.div key={supp.id}
                ...
                style={{
                  ...
                  boxShadow: taken ? '0 0 24px rgba(34,197,94,0.08)' : 'none'
                }}
              >
                <div>
                  <p style={{ fontSize: '15px', fontWeight: 600, color: taken ? '#22C55E' : '#F5F5F5', marginBottom: '3px', transition: 'color 0.3s' }}>{supp.name}</p>
                ...
                >{taken ? '✅ Taken' : 'Mark'}</motion.button>
              </motion.div>
            );
          })}
        </AnimatePresence>
